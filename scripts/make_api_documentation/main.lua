...
if _G.MiniDoc == nil then doc.setup() end
...
local function _get_last_numeric_key(data)
  ...
  for key, _ in pairs(data) do
    if type(key) ~= 'number' then
      if not found then error('No number key could be found.') end
...
local function _set_trailing_newline(section, count)
  ...
  for _, line in ipairs(section) do
    if not found_text then
      if _is_not_whitespace(line) then found_text = true end
...
local function _get_module_enabled_hooks(module_identifier)
  ...
  hooks.sections['@signature'] = function(section)
    if module_identifier then _strip_function_identifier(section, module_identifier) end
  ...
  hooks.sections['@tag'] = function(section)
    if module_identifier and module_name then _replace_function_name(section, module_identifier, module_name) end
  ...
  hooks.block_post = function(block)
    ...
    _apply_recursively(function(section)
      ...
      if section.info.id == '@field' and _is_field_section(section[1]) then
        ...
        if previous_section then _set_trailing_newline(section) end
      end

      if section.info.id == '@param' and _is_parameter_section(section[1]) then
        local previous_section = section.parent[section.parent_index - 1]

        if previous_section then _set_trailing_newline(previous_section) end
      end
...
local function main()
  ...
  local paths = {
    {
      source = vim.fs.joinpath(root, 'lua', 'lua-console', 'utils.lua'),
      destination = vim.fs.joinpath(root, 'doc', 'lua-console.nvim_api.txt'),
    },
  }
