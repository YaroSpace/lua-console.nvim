...
- The language evaluator is determined either from (in order of precedence):
  ...
  1. The code prefix `===lang` on the line above your code snippet, in which case it only applies to the snippet directly below and it should be included in the selection 
  for evaluation. The prefix can be changed in the config.
  2. The code prefix on the top line of the console/buffer, in which case it applies to the whole buffer.
  3. The file type of the buffer. 
